...
export default function AlternativeButton({ label, text, selected, striked, correct, wrong, locked, onSelect, onToggleStrike }: Props) {
  ...
  if (isPreAnswerSelected) cls += "border-blue-700 bg-blue-100 text-blue-950 ring-2 ring-blue-400 shadow-md ";
  ...
  return (
    <button aria-label={`Alternativa ${label}`} disabled={locked} className={cls} onClick={onSelect} onDoubleClick={onToggleStrike}>
      <div className="flex items-start gap-3">
        <span className={`inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold ${isPreAnswerSelected ? "bg-blue-700 text-white" : "bg-slate-100"}`}>
          {label}
        </span>
        <span>{text}</span>
      </div>
